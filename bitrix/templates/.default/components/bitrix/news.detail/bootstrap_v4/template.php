...
<div class="news-detail<?=$themeClass?>">
	<div class="mb-3" id="<?echo $this->GetEditAreaId($arResult['ID'])?>">
		...
		<div class="news-detail-body">
			...
			<!-- Кастомная навигация между новостями в детальной новости -->

			<?php if ($arResult['CUSTOM_NAVIGATION']['SHOW']) { ?>
				<div class="news-detail-custom-navigation">
					<div class="news-detail-custom-navigation-button <?= !empty($arResult['CUSTOM_NAVIGATION']['PREVIOUS']) ? 'active' : 'inactive' ?>">
						<a <?= !empty($arResult['CUSTOM_NAVIGATION']['PREVIOUS']['DETAIL_PAGE_URL']) ? 'href="' . $arResult['CUSTOM_NAVIGATION']['PREVIOUS']['DETAIL_PAGE_URL'] . '"' : '' ?>>
							Предыдущяя новость
						</a>
					</div>
					<div class="news-detail-custom-navigation-button <?= !empty($arResult['CUSTOM_NAVIGATION']['NEXT']) ? 'active' : 'inactive' ?>">
						<a <?= !empty($arResult['CUSTOM_NAVIGATION']['NEXT']['DETAIL_PAGE_URL']) ? 'href="' . $arResult['CUSTOM_NAVIGATION']['NEXT']['DETAIL_PAGE_URL'] . '"' : '' ?>>
							Следующая новость
						</a>
					</div>
				</div>
